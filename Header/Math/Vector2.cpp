...
void Vector2::Reset() { *this = ZeroVector2; }

Vector2 Vector2::Rotate(const float& theta) const { return *this * MakeRotateMatrix(theta); }

float Vector2::Length() const { return sqrtf(powf(x, 2) + powf(y, 2)); }
...
Vector2 Vector2::operator*(const Matrix2x2& Second) const {
	const Vector2 b[2]{
	    {Second.m[0][0], Second.m[1][0]},
        {Second.m[0][1], Second.m[1][1]}
    };
	return Vector2{(*this * b[0]), (*this * b[1])};
}
